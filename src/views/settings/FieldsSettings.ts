...
import { ActivityDateFieldConfig, FieldDefinition, FieldsSettings } from 'src/types';
...
export function renderFieldSettings(
	...
	FIELD_DEFINITIONS.forEach((field) => {
		const contentEl = accordion.renderAccordionField(fieldGroupsContainer, field);
...
function addFieldSettings(
	...
	if (field.key !== 'firstRead' && field.key !== 'lastRead' && field.key !== 'review') {
			...
			.addText((text) =>
				text
					...
					.onChange(async (value) => {
						plugin.settings.fieldsSettings[field.key].propertyName = value || field.key;
	...
	if (field.hasDataSource) {
		const sourceKey = `${field.key}Source` as keyof typeof plugin.settings.dataSourcePreferences;
	...
	if (field.key === 'firstRead' || field.key === 'lastRead') {
		addActivityDatePropertyField(containerEl, field.key, 'start', field.name, plugin);
		addActivityDatePropertyField(containerEl, field.key, 'end', field.name, plugin);
	}
...
function addActivityDatePropertyField(
	...
	const fieldSettings = plugin.settings.fieldsSettings[fieldKey] as ActivityDateFieldConfig;
	...
	new Setting(containerEl)
		...
		.addText((text) => {
			text
				...
				.onChange(async (value) => {
					(plugin.settings.fieldsSettings[fieldKey] as ActivityDateFieldConfig)[propName] =
						value || defaultValue;
					await plugin.saveSettings();
				});
		});
